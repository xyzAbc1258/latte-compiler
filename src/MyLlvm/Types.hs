...
data LlvmLit
  ...
  | LMUndefLit LlvmType
  -- | Zero initializer value
  | LMZeroInitializer LlvmType
  deriving (Eq)
...
-- failure.
ppLit (LMUndefLit t    )       = text "undef" --sdocWithDynFlags f
  where f dflags
          | gopt Opt_LlvmFillUndefWithGarbage dflags,
            Just lit <- garbageLit t   = ppLit lit
          | otherwise                  = text "undef"
ppLit (LMZeroInitializer _)    = text "zeroinitializer"
...
getLitType (LMZeroInitializer t) = t
