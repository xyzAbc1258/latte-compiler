...
ppLlvmExpression expr
  = case expr of
        ...
        InsertV    struct val idx   -> ppInsertV struct val idx
        ...
        Select     cond ift iff     -> ppSelect cond ift iff
...
ppCall ct fptr args attrs = case fptr of
    ...
    LMNLocalVar _ (LMPointer (LMFunction d)) -> ppCall' d
...
ppInsertV :: LlvmVar -> LlvmVar -> [Integer] -> SDoc
ppInsertV struct val idx =
  text "insertvalue"
  <+> ppr (getVarType struct) <+> ppName struct <> comma
  <+> ppr (getVarType val) <+> ppName val <> comma
  <+> foldl1 (<+>) (map (text . show) idx)
...
ppSelect cond ift iff =
  text "select"
  <+> ppr (getVarType cond) <+> ppName cond <> comma
  <+> ppr (getVarType ift) <+> ppName ift <> comma
  <+> ppr (getVarType iff) <+> ppName iff
