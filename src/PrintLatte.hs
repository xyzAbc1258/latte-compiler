...
instance Print (TopDef a) where
  prt i e = case e of
    ...
    Struct _ id types -> prPrec i 0 (concatD [prt 0 id, prt 0 types])
...
instance Print (Type a) where
  prt i e = case e of
    ...
    Array _ type_ -> prPrec i 0 (concatD [prt 0 type_, doc (showString "["), doc (showString "]")])
    Class _ id -> prPrec i 2 (concatD [prt 0 id])
...
instance Print (Expr a) where
  prt i e = case e of
    ...
    ENewObj _ type_ -> prPrec i 7 (concatD [doc (showString "new"), prt 2 type_])
    ENewArr _ type_ expr -> prPrec i 7 (concatD [doc (showString "new"), prt 0 type_, doc (showString "["), prt 3 expr, doc (showString "]")])
    ...
    ENull _ -> prPrec i 7 (concatD [doc (showString "null")])
    ECast _ type_ expr -> prPrec i 6 (concatD [doc (showString "("), prt 0 type_, doc (showString ")"), prt 7 expr])
    ...
    EVirtCall _ expr n exprs -> prPrec i 0 (concatD [prt 0 expr, prt 0 n, prt 0 exprs])
    EFldNoAcc _ expr n -> prPrec i 0 (concatD [prt 0 expr, prt 0 n])
  prtList _ [] = (concatD [])
  prtList _ [x] = (concatD [prt 0 x])
  prtList _ (x:xs) = (concatD [prt 0 x, doc (showString ","), prt 0 xs])
